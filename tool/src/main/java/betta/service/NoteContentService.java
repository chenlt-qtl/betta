...
import betta.mapper.NoteContentMapper;
import betta.domain.AjaxResult;
import betta.utils.ImgUtil;
...
public class NoteContentService {
    ...
    public void backupImg() {
        ...
        for (; total > (pageNo - 1) * pageSize; pageNo++) {
            ...
            AjaxResult result = restService.get("/note/content/list", param);
            List<Map> records = (List<Map>) result.get("rows");
...
//                String text = BtoaEncode.decrypt((String) noteContent.get("text"));
                String text = (String) noteContent.get("text");

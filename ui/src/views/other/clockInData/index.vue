    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['other:clockInData:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['other:clockInData:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['other:clockInData:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['other:clockInData:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listClockInData, getClockInData, delClockInData, addClockInData, updateClockInData } from "@/api/other/clockInData";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getClockInData(id).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除打卡数据编号为"' + ids + '"的数据项？').then(function() {
        return delClockInData(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('other/clockInData/export', {
        ...this.queryParams
      }, `clockInData_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
